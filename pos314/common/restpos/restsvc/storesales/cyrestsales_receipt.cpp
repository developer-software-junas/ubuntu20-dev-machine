...
//*******************************************************************
bool CYRestReceipt::assembleReceipt(Json::Value& jheader,
                                    Json::Value& jdetail,
                                    Json::Value& jpayment,
...
// DESCRIPTION:     Create a receipt
//*******************************************************************
bool                CYRestReceipt::assembleReceiptGiftWrap (Json::Value& jheader,
                                                            Json::Value& jdetail)
{
    _txtgift = "";
    if (_isGiftWrap) {
        ...
        _txtgift += "\n\n";
        _txtgift += _util->cyAcrossText(w, "-");
        _txtgift += _util->cyCenterText(w, "*** GIFT RECEIPT ***");
        _txtgift += _util->cyAcrossText(w, "-");
        ...
        _txtgift += _util->cyCenterText(w, tmp);
        _txtgift += "\n";

        tmp = "Invoice Number ";
        tmp += legacyTransnum(jheader);
        _txtgift += _util->cyCenterText(w,tmp);
        _txtgift += _util->cyCenterText(w," ");
        ...
        _txtgift += " Cashier   : ";
        _txtgift += jcashier["first_name"].asString();
        _txtgift += " ";
        _txtgift += jcashier["last_name"].asString();
        if (len > 40)  {
            _txtgift += "\n           [";
            _txtgift += jcashier["login"].asString();
            _txtgift += "]\n";
        }  else  {
            _txtgift += "[";
            _txtgift += jcashier["login"].asString();
        ...
        _txtgift += " Date-Time : ";
        _txtgift += _util->date_pos();
        _txtgift += "\n";

        _txtgift += _util->cyAcrossText(w, "-");
        _txtgift += _util->cyPostSpace(14, "DEPARTMENT");
        _txtgift += _util->cyPreSpace (12, "SKU");
        _txtgift += _util->cyPreSpace (14, "QUANTITY");
        _txtgift += "\n";
        _txtgift += _util->cyAcrossText(40, "-");
            ...
            if (jline["gift_wrap_quantity"] != Json::nullValue)  {
                ...
                if (tmp.length() > 0)  {
                    if (_util->valid_decimal("Gift wrap quantity", tmp.c_str (), 1, 8, 1.00, 9999.00))  {
                        double dqty = _util->stodsafe(tmp);
                        sprintf(sztmp,"%.02f",dqty);

                        Json::Value jproduct;
                        jproduct = jline["detail_product_header"];
                        _txtgift += _util->cyPostSpace(14, jproduct["category_cd"].asString());
                        _txtgift += _util->cyPreSpace (12, jline["item_code"].asString());
                        _txtgift += _util->cyPreSpace (14, sztmp);
                        _txtgift += "\n";
                    }
                }
            }
        ...
        _txtgift += "\n\n\n";
        _txtgift += _util->cyCenterText(w, "Important!");
        _txtgift += _util->cyCenterText(w, "Present your gift receipt to exchange");
        _txtgift += _util->cyCenterText(w, "within 15 days from purchase.  Goods");
        _txtgift += _util->cyCenterText(w, "are not refundable for cash and");
        _txtgift += _util->cyCenterText(w, "Rustans will not accept used,");
        _txtgift += _util->cyCenterText(w, "damaged or altered goods for");
        _txtgift += _util->cyCenterText(w, "exchange.");
        _txtgift += "\n\n\n\n\n\n";
    }
    _receipt += _txtgift;
    return true;
...
// DESCRIPTION:     Create a receipt
//*******************************************************************
bool                CYRestReceipt::assembleReceiptFooter (Json::Value& jheader,
                                                          salesReceiptType receiptType)
{
    _txtfoot = "";
...
// DESCRIPTION:     Create a receipt
//*******************************************************************
bool                CYRestReceipt::assembleReceiptSignature (Json::Value& jheader)
{
    _txtsig = "";
...
// DESCRIPTION:     Create a receipt
//*******************************************************************
bool                CYRestReceipt::assembleVatBreakdown (salesReceiptType receiptType)
{
    _txtvat = "";
    int w = g_widthReceipt;
...
// DESCRIPTION:     Create a receipt
//*******************************************************************
bool CYRestReceipt::assembleReceiptAccount (Json::Value& jheader)
{
    _txtacct = "";
    string info, label;
    bool isFsp = false;
    ...
    for (int i = 0; i < j; i++)  {
        ...
        if (isFsp || isBeauty) {
            ...
            string tmp = jacct["points_earned"].asString();
            if (_util->valid_decimal("Points earned",tmp.c_str(),1,24,0.01,99999999999.99))  {
                info  = "Points balance: ";
                info += _util->fmt_number_comma(tmp.c_str());
                _txtacct += _util->cyCenterText(w, info);
            }
          ...
          string tmp = jacct["points_earned"].asString();
          if (_util->valid_decimal("Points earned",tmp.c_str(),1,24,0.01,999999999.99))  {
              info  = "Points balance: ";
              info += tmp;
              _txtacct += _util->cyCenterText(w, info);
          }
...
// DESCRIPTION:     Create a receipt
//*******************************************************************
bool CYRestReceipt::assembleReceiptPayment (Json::Value& jheader,
                                            Json::Value& jpayment,
                                            salesReceiptType receiptType)
{
    string tmp;
    ...
    double netSalesAmount = _totalGrossAmount -
            (_totalItemDiscount+_totalTrxDiscount+_totalAmtExempt);
    double totalChangeAmount = totalPaymentAmount - netSalesAmount;
...
// DESCRIPTION:     Create a receipt
//*******************************************************************
bool CYRestReceipt::assembleReceiptTotals (Json::Value& jheader)
{
    char sztmp [32];
    _txttot = "";
    //////////////////////////////////////
    ...
    if (_isPwd || _isSenior)  {
        ...
        _txttot += _util->cyLRText(23,"Net sales amount",18,netSales);
        //  VAT exempt
...
// DESCRIPTION:     Create a receipt
//*******************************************************************
bool CYRestReceipt::assembleReceiptDetail (Json::Value& jdetail,
                                           Json::Value& jhdrtrx,
                                           salesReceiptType type)
{
    //_txtdet = "";
    Json::Value jdetprodhdr = jdetail["detail_product_header"];

    string tmp = jdetail["quantity"].asString();
    double quantity = _util->stodsafe(jdetail["quantity"].asString());
    double priceOriginal = _util->stodsafe(jdetail["retail_price"].asString());
    ...
    /*if (jdetail["consign_reference"].asString().length() > 0)  {
        ...
        tmp += "-";
        tmp += jdetprodhdr["base_type_code"].asString();
        tmp += "-";
        tmp += _util->string_cleanse(jdetprodhdr["description"].asString().c_str (),true);
    ...
    if (jdetail["det_modifier"]!=Json::nullValue)  {
        ...
        for (int i = 0; i < j; i++)  {
            ...
            if (!_util->valid_decimal("Add on amount", addonamt.c_str(), 1, 12, 0.00, 999999999))
                return seterrormsg(_util->errormsg());
            double damtadd = _util->stodsafe(addonamt);
            if (damtadd > 0)  {
                tmp  = "  ";
                ...
                if (!_util->valid_decimal("Add on quantity", addonqty.c_str(), 1, 12, 0.01, 999999999))
                    return seterrormsg(_util->errormsg());
                double damtqty = _util->stodsafe(addonqty);
    ...
    if (jdetail["item_discount"]!=Json::nullValue)  {
        ...
        string tmp = jdetail["item_discount_amount"].asString();
        if (_util->stodsafe(tmp) != 0)  {
            sprintf (sztmp, "%.02f", (_util->stodsafe(tmp)*1000)/1000);
            tmp = _util->cyLRText(28, desc, 12, sztmp);
            _txtdet += tmp;
        }
    }/*  else if (jdetail["det_discount"]!=Json::nullValue)  {
        string desc  = "  Less ";
        Json::Value jdisc = jdetail["det_discount"];
        desc += jdisc["description"].asString();
        tmp = jdisc["discount_value"].asString();
        sprintf (sztmp, "%.02f", (_util->stodsafe(tmp)*1000)/1000);
        tmp = _util->cyLRText(28, desc, 12, sztmp);
        _txtdet += tmp;
    }*/
...
// DESCRIPTION:     Create a receipt
//*******************************************************************
bool CYRestReceipt::assembleReceiptHeader (Json::Value& jheader,
                                         salesReceiptType receiptType)
{
    _txthdr = "";
    _receipt = "";
    ...
    if (jtemp["customer_id"].asString().length () >0)  {
        ...
        //if (jtemp2["is_sc"].asString() == "1" ||
            ...
            if (jtemp["account_type_desc"].asString().length() > 0)  {
                tmp = jtemp["account_type_desc"].asString ();

                _txthdr += "   Account     : ";
                _txthdr += tmp.substr(0,22);
                _txthdr += "\n";
            }
            ...
            if (jtemp["last_name"].asString().length() > 0)  {
                _txthdr += "   Name        : ";
                tmp  = jtemp["first_name"].asString ();
                tmp += " ";
                tmp += jtemp["last_name"].asString ();
                tmp  = tmp.substr(0,23);
                tmp += "\n";
    ...
    if (receiptType == salesReceiptType::SRT_REPRINT) {
      ...
      _txthdr += _util->cyCenterText(w, jheader["remarks"].asString());
        ...
        if (jheader["ba_redemption_reference"].asString().length() < 1)  {
          _txthdr += _util->cyCenterText(w, " ");
    ...
    _txthdr += _util->cyCenterText(w, " ");
        ...
        if (receiptType == salesReceiptType::SRT_REPRINT)  {
            string strDate, strTime, fmtDate;
            char szdate [32], sztime[32];
            string trxDate = jheader["transaction_date"].asString();
            string trxTime = jheader["transaction_time"].asString();
            long lDate = atol (trxDate.c_str ());
            long lTime = atol(trxTime.c_str ());
            sprintf(szdate, "%08ld", lDate);
            sprintf(sztime, "%04ld", lTime);

            strDate = szdate;
            strTime = sztime;

            fmtDate  = strDate.substr(0,4);
            fmtDate += "/";
            fmtDate += strDate.substr(4,2);
            fmtDate += "/";
            fmtDate += strDate.substr(6,2);
            fmtDate += "-";
            fmtDate += strTime.substr(0,2);
            fmtDate += ":";
            fmtDate += strTime.substr(2,2);
/*
            memset(sztmp, 0x00, 30);
            strncpy (sztmp,szdate,4);
            fmtDate = sztmp;
            fmtDate += "/";
            memset(sztmp, 0x00, 30);
            strncpy (sztmp, szdate+4,2);
            fmtDate += sztmp;
            fmtDate += "/";
            memset(sztmp, 0x00, 30);
            strncpy (sztmp, szdate+6,2);
            fmtDate += sztmp;
            fmtDate += "-";
            memset(sztmp, 0x00, 30);
            ...
            fmtDate += ":";
            memset(sztmp, 0x00, 30);
            strncpy (sztmp, sztime+2,2);
            fmtDate += sztmp;
*/
            //    snprintf (tmp, 63, "%04d/%02d/%02d-%02d:%02d", 1900 + ltm->tm_year,
...
//*******************************************************************
CYRestReceipt::CYRestReceipt (cylog* log, cyini* ini, cycgi* cgi)
...
//*******************************************************************
bool CYRestReceipt::saveReceipt()
{
    return true;
}
