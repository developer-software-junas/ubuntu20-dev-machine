...
//*******************************************************************
bool CYReceiptSunnies::assembleReceiptPayment (Json::Value& jheader,
                                            Json::Value& jpayment,
                                            salesReceiptType receiptType)
{
    ...
    for (size_t i = 0; i < j; i++) {
        ...
        if (isGc) {
            ...
            if (isGc) {
                ...
                _txtpay += _util->cyLRText(left, jpay["description"].asString(), right, sztmp);
                int j = jgc.size();
                for (int i = 0; i < j; i++) {
                  gc.clear ();
                  gc = jgc[i];
                  ...
                  _txtpay += _util->cyLRText(left, "  OLD BALANCE", right, szold);

                  char sznew[128];
                  double dnew = stodsafe(gc["remaining_balance"].asString());
                  sprintf (sznew, "%.02f", dnew);
                  _txtpay += _util->cyLRText(left, "  NEW BALANCE", right, sznew);
            ...
            sright = FMTNumberComma(atof(sztmp));

            string sleft = "";
            sleft += jpay["description"].asString();
            sleft += "(";
            sleft += jpay["pay_code"].asString();
            sleft += ")";
            _txtpay += _util->cyLRText(left, sleft, right, sright);

            if (receiptType == SRT_REFUND)  {
                sleft  = "    RES NO.  : ";
                sleft += jpay["approval_code"].asString ();
                sleft += "\n";
                _txtpay += sleft;
        ...
        if (isCreditCard) {
            ...
            _txtpay += _util->cyLRText(left, lineDesc, right, jpay["approval_code"].asString());
        }
        ////////////////////////////////////
        //  Charge card
        lineDesc = "";
        ...
        if (receiptType != salesReceiptType::SRT_REFUND &&
            ...
            if (totalChangeAmount > 0) {
                changeText = _util->cyLRText(left, "Change", right, sztmp);
            }  else  {
                if (totalChangeAmount != 0)  {
    ...
    dtmp = _totalAmtZero;*/
    //_totalGrossAmount - (_totalItemDiscount+_totalTrxDiscount+_totalAmtExempt+_totalAmtZero);
    //////////////////////////////////////
