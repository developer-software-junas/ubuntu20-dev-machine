...
/********************************************************************
   ...
   DESCRIPTION:         Cleanup data
 ********************************************************************/
void                    cyerpsales::cleanup_customer ()
{
    //////////////////////////////////////
    //  Cleanup customer field(s)
    for (size_t i = 0; i < erpcustomer::ENUM_ERPCUSTOMER::si_end; i++)  {
        _customerInfo[i]="";
    }
}
/********************************************************************
                                              ...
                                              std::string &rawResult)
{
    ...
    request.setRawHeader("Authorization",tmp.c_str ());
...
/********************************************************************
   ...
   DESCRIPTION:         empty sales tables
 ********************************************************************/
std::string             cyerpsales::get_customer_id (std::string account_number)
{
    string sql;
    MySqlDb db (_pos);
    MySqlDb* posdb = &db;
    ...
    _error = "";
    posdb->sql_reset ();
    sql  = "select customer_id from sc_customer_card where card_number = '";
    sql += account_number; sql += "' ";
    if (posdb->sql_result(sql,false))  {
        if (!posdb->eof())  {
            _error = posdb->sql_field_value("customer_id");
        }
    }
    return _error;
}
/********************************************************************
   ...
   DESCRIPTION:         empty sales tables
 ********************************************************************/
bool                    cyerpsales::customer_exists (std::string account_number)
{
    string sql;
    MySqlDb db (_pos);
    MySqlDb* posdb = &db;
    ...
    _error = "";
    posdb->sql_reset ();
    sql  = "select * from sc_customer where customer_id = '";
    sql += account_number; sql += "' ";
    if (posdb->sql_result(sql,false))  {
        if (!posdb->eof())  {
            return true;
