...
/********************************************************************
      FUNCTION:         export_detail_tax
   DESCRIPTION:         Sales tax details
 ********************************************************************/
bool                    cyerpsales::export_detail_tax (MySqlDb* taxdb,
                                                       MySqlDb* detdb,
                                                       std::string systransnum)
{
    ...
    if (taxdb->eof())
        return seterrormsg("1-export_detail_tax::No item tax...");
    ...
    do  {
        ...
        if (!detdb->sql_result(sql,false))
            return seterrormsg(taxdb->errordb());
        if (detdb->eof())
            return seterrormsg("2-export_detail_tax::No item tax...");
        ...
        do  {
            ...
            string tax_desc  = detdb->sql_field_value("tax_desc");
